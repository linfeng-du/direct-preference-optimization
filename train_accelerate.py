...
def worker_main(accelerator,rank: int, world_size: int, config: DictConfig, policy: nn.Module, reference_model: Optional[nn.Module] = None):
    ...
    if config.use_hnet and not config.train_reward: 
            ...
            # hypernet = HyperNet(config.hnet.alpha, config.hnet.dropout, config.hnet.d_A,config.hnet.d_a,
            #                     config.hnet.d_hnet,
            #                     output_dim,config.hnet.d_emb,
            #                     config.hnet.n_transformer_layers,
            #                     configuration.num_hidden_layers ,config.hnet.n_transformer_heads)
