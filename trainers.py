...
from preference_datasets import get_batch_iterator,RLHFDataset
from utils import (
...
class BasicTrainer(object):
    def __init__(self,accelerator:Accelerator.__class__, policy: nn.Module, config: DictConfig, seed: int, run_dir: str, reference_model: Optional[nn.Module] = None, rank: int = 0, world_size: int = 1, hnet_controller = None,**kwargs):
        ...
        self.seed = seed
        # self.rank = rank
        self.rank = accelerator.device.index
        ...
        self.optimizer_non_loaded = getattr(torch.optim, self.config.optimizer)([ p for p in self.policy.parameters() if p.requires_grad], lr=self.config.lr)
        # else:
        #     self.optimizer_non_loaded = getattr(torch.optim, self.config.optimizer)(self.policy.parameters(), lr=self.config.lr)


        self.scheduler = torch.optim.lr_scheduler.LambdaLR(self.optimizer_non_loaded, lr_lambda=lambda step: min(1.0, (step + 1) / (self.config.warmup_steps + 1)))
        ...
        rank0_print(f'Loaded {len(self.eval_iterator)} eval batches of size {config.eval_batch_size}')
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], loss_config: DictConfig, train=True):
        ...
        if loss_config.name in {'dpo', 'ipo'}:
            ...
            # chosen_rewards = all_gather_if_needed(chosen_rewards, self.accelerator.device, self.world_size)

            # # exit()
            # rejected_rewards = all_gather_if_needed(rejected_rewards, self.accelerator.device, self.world_size)
            # reward_accuracies = all_gather_if_needed(reward_accuracies, self.accelerator.device, self.world_size)

            # print(f"{self.accelerator=}")
    ...
    def train(self):
        ...
        def evaluate():
                ...
                if self.config.use_hnet and self.first_pass:
...
class TensorParallelTrainer(BasicTrainer):
    def __init__(self, policy, config, seed, run_dir, reference_model=None, rank=0, world_size=1,hnet_controller=None ):
        """A trainer subclass that uses TensorParallel to shard the model across multiple GPUs.
...
class FSDPLoRATrainer(FSDPTrainer):
    def __init__(self, policy: nn.Module, config: DictConfig, seed: int, run_dir: str, reference_model: Optional[nn.Module] = None, rank: int = 0, world_size: int = 1, hnet_controller = None):
        """A trainer subclass that uses PyTorch FSDP to shard the hypernetwork and policy model across multiple GPUs.
